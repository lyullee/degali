...
def main():
    ...
    origin=ref/'phase_consistent_flux_witness_2026-09-06.json'
    ...
    paths=[ROOT/n for n in upstream['sha256']]+[origin,Path(__file__).resolve(),
        ROOT/'src/degali/addons/paired_moment_difference.py',ROOT/'tests/test_paired_moment_difference.py',
        ROOT/'docs/prereg-paired-energy-difference.md']
    ...
    data=dict(phase='paired_actual_energy_difference',completed=False,rows=[],sha256=initial_hashes,
    ...
    try:
        for factor in (.5,1.,2.,4.):
            ...
            out=paired_moment_difference(p,par,rates,h*factor,order=8,angular_order=8,batch_size=8)
            ...
            print('paired factor',factor,'energy',out['energy_error'],'other',out['nonenergy_maximum_error'],flush=True)
    ...
    print('paired same-step result',data['same_step_pair_passed'],data.get('failure'),flush=True)
